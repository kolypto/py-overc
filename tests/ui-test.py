...
class UITest(ApplicationTest, unittest.TestCase):
    ...
    def test_api_refresh(self):
        ...
        res, rv = self.test_client.jsonapi('GET', '/ui/api/refresh')
        self.assertEqual(rv.status_code, 200)
        ...
        self.assertEqual(services[0], {'id': 1, 'period': 60, 'timed_out': False, 'name': 'app',   'title': u'app',   'state': {'rtime': '2014-01-01 00:00:00', 'state': 'OK', 'info': 'Fine'}})
        self.assertEqual(services[1], {'id': 2, 'period': 60, 'timed_out': False, 'name': 'db',    'title': u'db',    'state': {'rtime': '2014-01-01 00:00:00', 'state': 'OK', 'info': 'Available'}})

        # Test server 2
        server = res['servers'][1]
        services = server.pop('services')
        self.assertEqual(services[0], {'id': 3, 'period': 90, 'timed_out': False, 'name': 'nginx', 'title': u'nginx', 'state': {'rtime': '2014-01-01 00:00:00', 'state': 'OK', 'info': 'Running'}})
